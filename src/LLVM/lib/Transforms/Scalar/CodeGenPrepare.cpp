...
bool CodeGenPrepare::EliminateMostlyEmptyBlocks(Function &F) {
  ...
  for (Function::iterator I = ++F.begin(), E = F.end(); I != E; ) {
    ...
    if (BBI != BB->begin()) {
      ...
      if (!ISA_DEBUG_INFO_INTRINSIC(BBI) && !isa<PHINode>(BBI))
        continue;
    }
...
bool CodeGenPrepare::OptimizeMemoryInst(Instruction *MemoryInst, Value *Addr,
  ...
  if (SunkAddr) {
    ...
    if (SunkAddr->getType() != Addr->getType())
      SunkAddr = new BitCastInst(SunkAddr, Addr->getType(), InsertPt);
  } else {
    ...
    if (AddrMode.BaseReg) {
      ...
      if (V->getType()->isPointerTy())
        V = new PtrToIntInst(V, IntPtrTy, InsertPt);
    ...
    if (AddrMode.Scale) {
      ...
      if (V->getType() == IntPtrTy) {
        ...
        V = new TruncInst(V, IntPtrTy, InsertPt);
      } else {
        V = new SExtInst(V, IntPtrTy, InsertPt);
      }
      if (AddrMode.Scale != 1)
        V = BinaryOperator::CreateMul(V, ConstantInt::get(IntPtrTy,
                                      ...
                                      InsertPt);
      if (Result)
        Result = BinaryOperator::CreateAdd(Result, V, InsertPt);
      else
        Result = V;
    }
    ...
    if (Result == 0)
      ...
      SunkAddr = new IntToPtrInst(Result, Addr->getType(), InsertPt);
  }
