  ...
  case Instruction::AShr:
    ...
    if (Op->hasOneUse()) {
      ...
      if (C == AndRHS) {          // Masking out bits shifted in.
        ...
        ShVal = Builder->CreateLShr(ShVal, OpRHS);
        return BinaryOperator::CreateAnd(ShVal, AndRHS);
      }
    }
    break;
  }
...
Value *InstCombiner::InsertRangeTest(Value *V, Constant *Lo, Constant *Hi,
  ...
  if (Inside) {
    ...
    Value *Add = Builder->CreateAdd(V, NegLo);
...
Value *InstCombiner::FoldLogicalPlusAnd(Value *LHS, Value *RHS,
  ...
  if (isSub)
    return Builder->CreateSub(LHSI->getOperand(0), RHS);
  return Builder->CreateAdd(LHSI->getOperand(0), RHS);
}
  ...
  case ICmpInst::ICMP_NE:
    ...
    case ICmpInst::ICMP_NE:
      if (LHSCst == SubOne(RHSCst)){// (X != 13 & X != 14) -> X-13 >u 1
        ...
        Value *Add = Builder->CreateAdd(Val, AddCST);
    ...
    if (CastInst *Op1C = dyn_cast<CastInst>(Op1)) {
      ...
      if (Op0C->getOpcode() == Op1C->getOpcode() && // same cast kind ?
        ...
        if (ShouldOptimizeCast(Op0C->getOpcode(), Op0COp, I.getType()) &&
          ...
          Value *NewOp = Builder->CreateAnd(Op0COp, Op1COp);
...
Instruction *InstCombiner::visitOr(BinaryOperator &I) {
  ...
  if (match(Op0, m_And(m_Value(A), m_Value(C))) &&
    ...
    if (C1 && C2) {  // (A & C1)|(B & C2)
      ...
      if ((C1->getValue() & C2->getValue()) == 0) {
        ...
        if (match(A, m_Or(m_Value(V1), m_ConstantInt(C3))) &&
          ...
          V2 = Builder->CreateOr(V1, ConstantExpr::getOr(C3, C4));
    ...
    if (Op0->hasOneUse() || Op1->hasOneUse()) {
      ...
      if (V1) {
        Value *Or = Builder->CreateOr(V2, V3);
      ...
      if (Op0C->getOpcode() == Op1C->getOpcode()) {// same cast kind ?
        ...
        if (SrcTy == Op1C->getOperand(0)->getType() &&
          ...
          if ((!isa<ICmpInst>(Op0COp) || !isa<ICmpInst>(Op1COp)) &&
              ...
              ShouldOptimizeCast(Op1C->getOpcode(), Op1COp, I.getType())) {
            Value *NewOp = Builder->CreateOr(Op0COp, Op1COp);
            return CastInst::Create(Op0C->getOpcode(), NewOp, I.getType());
          }
...
Instruction *InstCombiner::visitXor(BinaryOperator &I) {
    ...
    if (BinaryOperator *Op0I = dyn_cast<BinaryOperator>(NotOp)) {
      if (Op0I->getOpcode() == Instruction::And || 
        ...
        if (isFreeToInvert(Op0I->getOperand(0)) && 
          ...
          Value *NotX =
            Builder->CreateNot(Op0I->getOperand(0));
          Value *NotY =
            Builder->CreateNot(Op0I->getOperand(1));
          if (Op0I->getOpcode() == Instruction::And)
  ...
  if (Op0I && Op1I) {
    ...
    if ((Op0I->hasOneUse() || Op1I->hasOneUse()) &&
      ...
      if (X) {
        Value *NewOp = Builder->CreateXor(Y, Z);
