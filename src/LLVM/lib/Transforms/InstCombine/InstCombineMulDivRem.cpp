  ...
  if (Constant *Op1C = dyn_cast<Constant>(Op1)) {
    ...
    if (BinaryOperator *Op0I = dyn_cast<BinaryOperator>(Op0))
      if (Op0I->getOpcode() == Instruction::Add && Op0I->hasOneUse() &&
        ...
        Value *Add = Builder->CreateMul(Op0I->getOperand(0), Op1C);
    ...
    if (RHSI->getOpcode() == Instruction::Shl &&
      ...
      if (C1.isPowerOf2()) {
        ...
        if (uint32_t C2 = C1.logBase2())
          N = Builder->CreateAdd(N, ConstantInt::get(NTy, C2));
    ...
    if (ConstantInt *STO = dyn_cast<ConstantInt>(SI->getOperand(1)))
      if (ConstantInt *SFO = dyn_cast<ConstantInt>(SI->getOperand(2)))  {
        ...
        if (TVA.isPowerOf2() && FVA.isPowerOf2()) {
          ...
          Value *TSI = Builder->CreateLShr(Op0, TC);
  
          // Construct the "on false" case of the select
          Constant *FC = ConstantInt::get(Op0->getType(), FSA); 
          Value *FSI = Builder->CreateLShr(Op0, FC);

          // construct the select instruction and return it.
          return SelectInst::Create(SI->getOperand(0), TSI, FSI);
        }
      }
  return 0;
}
...
Instruction *InstCombiner::visitSDiv(BinaryOperator &I) {
  ...
  if (ConstantInt *RHS = dyn_cast<ConstantInt>(Op1)) {
    ...
    if (cast<SDivOperator>(&I)->isExact() &&
      ...
      return BinaryOperator::CreateAShr(Op0, ShAmt);
    }
  ...
  if (I.getType()->isIntegerTy()) {
    ...
    if (MaskedValueIsZero(Op0, Mask)) {
      if (MaskedValueIsZero(Op1, Mask)) {
        ...
        return BinaryOperator::CreateUDiv(Op0, Op1);
      }
  ...
  if (Instruction *RHSI = dyn_cast<Instruction>(I.getOperand(1))) {
    ...
    if (RHSI->getOpcode() == Instruction::Shl &&
      ...
      if (cast<ConstantInt>(RHSI->getOperand(0))->getValue().isPowerOf2()) {
        ...
        Value *Add = Builder->CreateAdd(RHSI, N1);
...
Instruction *InstCombiner::visitSRem(BinaryOperator &I) {
  ...
  if (I.getType()->isIntegerTy()) {
    ...
    if (MaskedValueIsZero(Op1, Mask) && MaskedValueIsZero(Op0, Mask)) {
      ...
      return BinaryOperator::CreateURem(Op0, Op1);
    }
  }
