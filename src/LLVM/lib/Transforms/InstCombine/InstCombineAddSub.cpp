...
Instruction *InstCombiner::visitAdd(BinaryOperator &I) {
  ...
  if (Constant *RHSC = dyn_cast<Constant>(RHS)) {
    ...
    if (isa<ConstantInt>(RHSC) &&
      ...
      if (ExtendAmt) {
        ...
        Value *NewShl = Builder->CreateShl(XorLHS, ShAmt);
  ...
  if (Value *LHSV = dyn_castNegVal(LHS)) {
    if (LHS->getType()->isIntOrIntVectorTy()) {
      if (Value *RHSV = dyn_castNegVal(RHS)) {
        Value *NewAdd = Builder->CreateAdd(LHSV, RHSV);
  ...
  if (I.getType()->isIntOrIntVectorTy()) {
    ...
    if (match(LHS, m_Mul(m_Value(W), m_Value(X))) &&
      ...
      if (W == Y) {
        Value *NewAdd = Builder->CreateAdd(X, Z);
    ...
    if (LHS->hasOneUse() &&
      ...
      if (Anded == CRHS) {
        ...
        if (AddRHSHighBits == AddRHSHighBitsAnd) {
          ...
          Value *NewAdd = Builder->CreateAdd(X, CRHS);
...
Value *InstCombiner::EmitGEPOffset(User *GEP) {
  ...
  for (User::op_iterator i = GEP->op_begin() + 1, e = GEP->op_end(); i != e;
    ...
    if (Size != 1) {
      ...
      Op = Builder->CreateMul(Op, Scale);
    }

    // Emit an add instruction.
    Result = Builder->CreateAdd(Op, Result);
  }
  return Result;
}
  ...
  if (BinaryOperator *Op1I = dyn_cast<BinaryOperator>(Op1)) {
    ...
    if (Op1I->hasOneUse()) {
      ...
      if (Op1I->getOpcode() == Instruction::And &&
        ...
        Value *NewNot = Builder->CreateNot(OtherOp);
