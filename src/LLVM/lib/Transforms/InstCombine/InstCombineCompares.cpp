...
static Value *EvaluateGEPOffsetExpression(User *GEP, Instruction &I,
  ...
  if (Offset == 0) {
    ...
    if (VariableIdx->getType()->getPrimitiveSizeInBits() > IntPtrWidth)
      VariableIdx = new TruncInst(VariableIdx, 
                                  TD.getIntPtrType(VariableIdx->getContext()), &I);
    return VariableIdx;
  }
  ...
  return BinaryOperator::CreateAdd(VariableIdx, OffsetVal, &I);
}
  ...
  case Instruction::And:         // (icmp pred (and X, AndCST), RHS)
    if (LHSI->hasOneUse() && isa<ConstantInt>(LHSI->getOperand(1)) &&
      ...
      if (Shift && Shift->hasOneUse() && RHSV == 0 &&
        ...
        // Compute X & (C << Y).
        Value *NewAnd = 
          Builder->CreateAnd(Shift->getOperand(0), NS);
  ...
  case Instruction::Shl: {       // (icmp pred (shl X, ShAmt), CI)
    ...
    if (ICI.isEquality()) {
      ...
      if (LHSI->hasOneUse()) {
        ...
        Value *And =
          Builder->CreateAnd(LHSI->getOperand(0),Mask);
...
Instruction *InstCombiner::visitICmpInstWithCastAndCast(ICmpInst &ICI) {
  ...
  if (isSignedCmp) {
    ...
    if (isSignedExt) {
      ...
      Result = Builder->CreateICmpSGT(LHSCIOp, NegOne);
    } else {
...
Instruction *InstCombiner::visitICmpInst(ICmpInst &I) {
  ...
  if (Ty->isIntegerTy(1)) {
    ...
    case ICmpInst::ICMP_EQ: {               // icmp eq i1 A, B -> ~(A^B)
      Value *Xor = Builder->CreateXor(Op0, Op1);
