...
User::op_iterator CallSite::getCallee() const {
  ...
  return cast<CallInst>(II)->op_end() - 1; // Skip Callee
}
...
AllocaInst::AllocaInst(const Type *Ty,
                       Instruction *InsertBefore)
  : UnaryInstruction(PointerType::getUnqual(Ty), Alloca,
...
LoadInst::LoadInst(Value *Ptr, Instruction *InsertBef)
...
LoadInst::LoadInst(Value *Ptr, BasicBlock *InsertAE)
  : UnaryInstruction(cast<PointerType>(Ptr->getType())->getElementType(),
                     Load, Ptr, InsertAE) {
  setVolatile(false);
  setAlignment(0);
...
LoadInst::LoadInst(Value *Ptr, bool isVolatile,
...
void ExtractValueInst::init(unsigned Idx) {
  assert(NumOperands == 1 && "NumOperands not initialized?");
...
BinaryOperator *BinaryOperator::Create(BinaryOps Op, Value *S1, Value *S2,
                                       BasicBlock *InsertAtEnd) {
  BinaryOperator *Res = Create(Op, S1, S2);
...
BinaryOperator *BinaryOperator::CreateNeg(Value *Op,
  ...
  return new BinaryOperator(Instruction::Sub,
                            zero, Op,
                            Op->getType(), InsertAtEnd);
}

BinaryOperator *BinaryOperator::CreateNSWNeg(Value *Op,
                                             Instruction *InsertBefore) {
  Value *zero = ConstantFP::getZeroValueForNegation(Op->getType());
  return BinaryOperator::CreateNSWSub(zero, Op, InsertBefore);
}

BinaryOperator *BinaryOperator::CreateNSWNeg(Value *Op,
                                             BasicBlock *InsertAtEnd) {
  Value *zero = ConstantFP::getZeroValueForNegation(Op->getType());
  return BinaryOperator::CreateNSWSub(zero, Op, InsertAtEnd);
}

BinaryOperator *BinaryOperator::CreateNUWNeg(Value *Op,
                                             Instruction *InsertBefore) {
  Value *zero = ConstantFP::getZeroValueForNegation(Op->getType());
  return BinaryOperator::CreateNUWSub(zero, Op, InsertBefore);
}

BinaryOperator *BinaryOperator::CreateNUWNeg(Value *Op,
                                             BasicBlock *InsertAtEnd) {
  Value *zero = ConstantFP::getZeroValueForNegation(Op->getType());
  return BinaryOperator::CreateNUWSub(zero, Op, InsertAtEnd);
}
...
CastInst *CastInst::Create(Instruction::CastOps op, Value *S, const Type *Ty, 
  ...
  switch (op) {
    case Trunc:    return new TruncInst    (S, Ty, InsertBefore);
    case ZExt:     return new ZExtInst     (S, Ty, InsertBefore);
    case SExt:     return new SExtInst     (S, Ty, InsertBefore);
    case FPTrunc:  return new FPTruncInst  (S, Ty, InsertBefore);
    case FPExt:    return new FPExtInst    (S, Ty, InsertBefore);
    case UIToFP:   return new UIToFPInst   (S, Ty, InsertBefore);
    case SIToFP:   return new SIToFPInst   (S, Ty, InsertBefore);
    case FPToUI:   return new FPToUIInst   (S, Ty, InsertBefore);
    case FPToSI:   return new FPToSIInst   (S, Ty, InsertBefore);
    case PtrToInt: return new PtrToIntInst (S, Ty, InsertBefore);
    case IntToPtr: return new IntToPtrInst (S, Ty, InsertBefore);
    case BitCast:  return new BitCastInst  (S, Ty, InsertBefore);
    default:
...
CastInst *CastInst::Create(Instruction::CastOps op, Value *S, const Type *Ty,
  ...
  // Construct and return the appropriate CastInst subclass
  switch (op) {
    case Trunc:    return new TruncInst    (S, Ty, InsertAtEnd);
    case ZExt:     return new ZExtInst     (S, Ty, InsertAtEnd);
    case SExt:     return new SExtInst     (S, Ty, InsertAtEnd);
    case FPTrunc:  return new FPTruncInst  (S, Ty, InsertAtEnd);
    case FPExt:    return new FPExtInst    (S, Ty, InsertAtEnd);
    case UIToFP:   return new UIToFPInst   (S, Ty, InsertAtEnd);
    case SIToFP:   return new SIToFPInst   (S, Ty, InsertAtEnd);
    case FPToUI:   return new FPToUIInst   (S, Ty, InsertAtEnd);
    case FPToSI:   return new FPToSIInst   (S, Ty, InsertAtEnd);
    case PtrToInt: return new PtrToIntInst (S, Ty, InsertAtEnd);
    case IntToPtr: return new IntToPtrInst (S, Ty, InsertAtEnd);
    case BitCast:  return new BitCastInst  (S, Ty, InsertAtEnd);
    default:
      assert(!"Invalid opcode provided");
  }
  return 0;
...
CastInst *CastInst::CreateZExtOrBitCast(Value *S, const Type *Ty, 
  ...
  if (S->getType()->getScalarSizeInBits() == Ty->getScalarSizeInBits())
    return Create(Instruction::BitCast, S, Ty, InsertBefore);
  return Create(Instruction::ZExt, S, Ty, InsertBefore);
}

CastInst *CastInst::CreateZExtOrBitCast(Value *S, const Type *Ty, 
                                        BasicBlock *InsertAtEnd) {
  if (S->getType()->getScalarSizeInBits() == Ty->getScalarSizeInBits())
    return Create(Instruction::BitCast, S, Ty, InsertAtEnd);
  return Create(Instruction::ZExt, S, Ty, InsertAtEnd);
}
...
CastInst *CastInst::CreateSExtOrBitCast(Value *S, const Type *Ty, 
  ...
  if (S->getType()->getScalarSizeInBits() == Ty->getScalarSizeInBits())
    return Create(Instruction::BitCast, S, Ty, InsertBefore);
  return Create(Instruction::SExt, S, Ty, InsertBefore);
}
...
CastInst *CastInst::CreateSExtOrBitCast(Value *S, const Type *Ty, 
  ...
  if (S->getType()->getScalarSizeInBits() == Ty->getScalarSizeInBits())
    return Create(Instruction::BitCast, S, Ty, InsertAtEnd);
  return Create(Instruction::SExt, S, Ty, InsertAtEnd);
}
...
CastInst *CastInst::CreateTruncOrBitCast(Value *S, const Type *Ty,
  ...
  if (S->getType()->getScalarSizeInBits() == Ty->getScalarSizeInBits())
    return Create(Instruction::BitCast, S, Ty, InsertBefore);
  return Create(Instruction::Trunc, S, Ty, InsertBefore);
}
...
CastInst *CastInst::CreateTruncOrBitCast(Value *S, const Type *Ty,
  ...
  if (S->getType()->getScalarSizeInBits() == Ty->getScalarSizeInBits())
    return Create(Instruction::BitCast, S, Ty, InsertAtEnd);
  return Create(Instruction::Trunc, S, Ty, InsertAtEnd);
}
...
CastInst *CastInst::CreatePointerCast(Value *S, const Type *Ty,
  ...
  if (Ty->isIntegerTy())
    return Create(Instruction::PtrToInt, S, Ty, InsertAtEnd);
  return Create(Instruction::BitCast, S, Ty, InsertAtEnd);
}
...
CastInst *CastInst::CreatePointerCast(Value *S, const Type *Ty, 
  ...
  if (Ty->isIntegerTy())
    return Create(Instruction::PtrToInt, S, Ty, InsertBefore);
  return Create(Instruction::BitCast, S, Ty, InsertBefore);
}
...
CastInst *CastInst::CreateIntegerCast(Value *C, const Type *Ty, 
  ...
  return Create(opcode, C, Ty, InsertBefore);
}
...
CastInst *CastInst::CreateIntegerCast(Value *C, const Type *Ty, 
  ...
  Instruction::CastOps opcode =
    (SrcBits == DstBits ? Instruction::BitCast :
     (SrcBits > DstBits ? Instruction::Trunc :
      (isSigned ? Instruction::SExt : Instruction::ZExt)));
  return Create(opcode, C, Ty, InsertAtEnd);
}
...
TruncInst::TruncInst(
  Value *S, const Type *Ty, Instruction *InsertBefore
) : CastInst(Ty, Trunc, S, InsertBefore) {
  assert(castIsValid(getOpcode(), S, Ty) && "Illegal Trunc");
}

TruncInst::TruncInst(
  Value *S, const Type *Ty, BasicBlock *InsertAtEnd
) : CastInst(Ty, Trunc, S, InsertAtEnd) { 
  assert(castIsValid(getOpcode(), S, Ty) && "Illegal Trunc");
}

ZExtInst::ZExtInst(
  Value *S, const Type *Ty, Instruction *InsertBefore
)  : CastInst(Ty, ZExt, S, InsertBefore) { 
  assert(castIsValid(getOpcode(), S, Ty) && "Illegal ZExt");
}

ZExtInst::ZExtInst(
  Value *S, const Type *Ty, BasicBlock *InsertAtEnd
)  : CastInst(Ty, ZExt, S, InsertAtEnd) { 
  assert(castIsValid(getOpcode(), S, Ty) && "Illegal ZExt");
}
SExtInst::SExtInst(
  Value *S, const Type *Ty, Instruction *InsertBefore
) : CastInst(Ty, SExt, S, InsertBefore) { 
  assert(castIsValid(getOpcode(), S, Ty) && "Illegal SExt");
}

SExtInst::SExtInst(
  Value *S, const Type *Ty, BasicBlock *InsertAtEnd
)  : CastInst(Ty, SExt, S, InsertAtEnd) { 
  assert(castIsValid(getOpcode(), S, Ty) && "Illegal SExt");
}

FPTruncInst::FPTruncInst(
  Value *S, const Type *Ty, Instruction *InsertBefore
) : CastInst(Ty, FPTrunc, S, InsertBefore) { 
  assert(castIsValid(getOpcode(), S, Ty) && "Illegal FPTrunc");
}

FPTruncInst::FPTruncInst(
  Value *S, const Type *Ty, BasicBlock *InsertAtEnd
) : CastInst(Ty, FPTrunc, S, InsertAtEnd) { 
  assert(castIsValid(getOpcode(), S, Ty) && "Illegal FPTrunc");
}

FPExtInst::FPExtInst(
  Value *S, const Type *Ty, Instruction *InsertBefore
) : CastInst(Ty, FPExt, S, InsertBefore) { 
  assert(castIsValid(getOpcode(), S, Ty) && "Illegal FPExt");
}

FPExtInst::FPExtInst(
  Value *S, const Type *Ty, BasicBlock *InsertAtEnd
) : CastInst(Ty, FPExt, S, InsertAtEnd) { 
  assert(castIsValid(getOpcode(), S, Ty) && "Illegal FPExt");
}

UIToFPInst::UIToFPInst(
  Value *S, const Type *Ty, Instruction *InsertBefore
) : CastInst(Ty, UIToFP, S, InsertBefore) { 
  assert(castIsValid(getOpcode(), S, Ty) && "Illegal UIToFP");
}

UIToFPInst::UIToFPInst(
  Value *S, const Type *Ty, BasicBlock *InsertAtEnd
) : CastInst(Ty, UIToFP, S, InsertAtEnd) { 
  assert(castIsValid(getOpcode(), S, Ty) && "Illegal UIToFP");
}

SIToFPInst::SIToFPInst(
  Value *S, const Type *Ty, Instruction *InsertBefore
) : CastInst(Ty, SIToFP, S, InsertBefore) { 
  assert(castIsValid(getOpcode(), S, Ty) && "Illegal SIToFP");
}

SIToFPInst::SIToFPInst(
  Value *S, const Type *Ty, BasicBlock *InsertAtEnd
) : CastInst(Ty, SIToFP, S, InsertAtEnd) { 
  assert(castIsValid(getOpcode(), S, Ty) && "Illegal SIToFP");
}

FPToUIInst::FPToUIInst(
  Value *S, const Type *Ty, Instruction *InsertBefore
) : CastInst(Ty, FPToUI, S, InsertBefore) { 
  assert(castIsValid(getOpcode(), S, Ty) && "Illegal FPToUI");
}

FPToUIInst::FPToUIInst(
  Value *S, const Type *Ty, BasicBlock *InsertAtEnd
) : CastInst(Ty, FPToUI, S, InsertAtEnd) { 
  assert(castIsValid(getOpcode(), S, Ty) && "Illegal FPToUI");
}

FPToSIInst::FPToSIInst(
  Value *S, const Type *Ty, Instruction *InsertBefore
) : CastInst(Ty, FPToSI, S, InsertBefore) { 
  assert(castIsValid(getOpcode(), S, Ty) && "Illegal FPToSI");
}

FPToSIInst::FPToSIInst(
  Value *S, const Type *Ty, BasicBlock *InsertAtEnd
) : CastInst(Ty, FPToSI, S, InsertAtEnd) { 
  assert(castIsValid(getOpcode(), S, Ty) && "Illegal FPToSI");
}

PtrToIntInst::PtrToIntInst(
  Value *S, const Type *Ty, Instruction *InsertBefore
) : CastInst(Ty, PtrToInt, S, InsertBefore) { 
  assert(castIsValid(getOpcode(), S, Ty) && "Illegal PtrToInt");
}

PtrToIntInst::PtrToIntInst(
  Value *S, const Type *Ty, BasicBlock *InsertAtEnd
) : CastInst(Ty, PtrToInt, S, InsertAtEnd) { 
  assert(castIsValid(getOpcode(), S, Ty) && "Illegal PtrToInt");
}

IntToPtrInst::IntToPtrInst(
  Value *S, const Type *Ty, Instruction *InsertBefore
) : CastInst(Ty, IntToPtr, S, InsertBefore) { 
  assert(castIsValid(getOpcode(), S, Ty) && "Illegal IntToPtr");
}

IntToPtrInst::IntToPtrInst(
  Value *S, const Type *Ty, BasicBlock *InsertAtEnd
) : CastInst(Ty, IntToPtr, S, InsertAtEnd) { 
  assert(castIsValid(getOpcode(), S, Ty) && "Illegal IntToPtr");
}

BitCastInst::BitCastInst(
  Value *S, const Type *Ty, Instruction *InsertBefore
) : CastInst(Ty, BitCast, S, InsertBefore) { 
  assert(castIsValid(getOpcode(), S, Ty) && "Illegal BitCast");
}

BitCastInst::BitCastInst(
  Value *S, const Type *Ty, BasicBlock *InsertAtEnd
) : CastInst(Ty, BitCast, S, InsertAtEnd) { 
  assert(castIsValid(getOpcode(), S, Ty) && "Illegal BitCast");
}
