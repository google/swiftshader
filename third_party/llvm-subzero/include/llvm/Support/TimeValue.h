...
public:
  ...
  static TimeValue MinTime() { return TimeValue(INT64_MIN, 0); }

  /// A constant TimeValue representing the largest time
  /// value permissible by the class. MaxTime is some point
  /// in the distant future, about 300 billion years AD.
  /// @brief The largest possible time value.
  static TimeValue MaxTime() { return TimeValue(INT64_MAX, 0); }
  ...
  static TimeValue ZeroTime() { return TimeValue(0, 0); }
...
public:
  ...
  explicit TimeValue(double new_time) : seconds_(0), nanos_(0) {
...
public:
  ...
  uint32_t microseconds() const { return nanos_ / NANOSECONDS_PER_MICROSECOND; }

  /// Returns only the fractional portion of the TimeValue rounded down to the
  /// nearest millisecond (divide by one million).
  /// @brief Retrieve the fractional part as milliseconds;
  uint32_t milliseconds() const { return nanos_ / NANOSECONDS_PER_MILLISECOND; }
  ...
  uint64_t toEpochTime() const { return seconds_ - PosixZeroTimeSeconds; }
